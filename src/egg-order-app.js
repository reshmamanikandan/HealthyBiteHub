...
import { Egg, Plus, Minus, Check, MapPin, Phone, User, RefreshCw, ChefHat, ClipboardList, Trash2 } from "lucide-react";
// Import Firebase SDKs (Ensure firebase package is installed: npm install firebase)
import { initializeApp } from "firebase/app";
import { getDatabase, ref, onValue, push, update, remove } from "firebase/database";

// 1. Replace with your actual Firebase project config details
...
.egg-app { font-family: 'Work Sans', sans-serif; background:#FAF6EE; color:#2E2318; min-height:100%; }
...
export default function EggOrderApp() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(ordersRef, (snapshot) => {
      ...
      if (data) {
        // Convert object dictionary from Firebase into an array
  ...
  const submitOrder = async (ev) => {
    ...
    try {
      ...
      // Push order directly to Firebase cloud database
      const ordersRef = ref(db, "orders");
    ...
    <div className="egg-app" style={{ padding: "0 0 40px" }}>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "28px 20px 0" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 38, height: 38, borderRadius: 10, background: "#F2A93B", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Egg size={20} color="#2E2318" strokeWidth={2.2} />
            </div>
            <div>
          ...
          confirmed ? (
            <div style={{ background: "#FFFFFF", border: "1.5px solid #E3D9C6", borderRadius: 16, padding: 28, textAlign: "center" }}>
              <div style={{ width: 52, height: 52, borderRadius: "50%", background: "#E8EEE4", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                <Check size={26} color="#4F6A4B" strokeWidth={2.5} />
              </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginBottom: 14 }}>
              {orders.length > 0 && (
                ...
                >
                  <Trash2 size={13} /> {clearArmed ? "Tap again to confirm" : "Start new day"}
                </button>
              )}
            </div>
        ...
        <div style={{ textAlign: "center", marginTop: 26, fontSize: 11, color: "#B3A692" }}>
          <ChefHat size={12} style={{ verticalAlign: "-2px", marginRight: 4 }} />
